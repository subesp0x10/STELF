...
def create_proc_as_sid(sid, prog):
 
    # If a user try to impersonate a user token
    if sid != "S-1-5-18":
        if not isSystem():
            raise Exception("[-]You need System privileges to impersonate a user token")
    else:
        # the user tries to getsystem
        if not windll.Shell32.IsUserAnAdmin():
            raise Exception("[-]You need admin rights to run getsystem!")
