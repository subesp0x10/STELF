...
def create_mutexes():
    admin_mutex = normal_mutex = None
    try:
        normal_mutex = mutex.NamedMutex(MUTEX_NAME, True)
    except WindowsError as e:
        e = str(e)
        print "Error creating normal mutex: "+e
        if not helpers.is_admin(): sys.exit(0)
         
    print "Created user mutex"
         
    if helpers.is_admin():
        try:
            admin_mutex = mutex.NamedMutex(ADMIN_MUTEX_NAME, True)
        except WindowsError as e:
            e = str(e)
            print "Error creating admin mutex: "+e
            sys.exit(0)
             
        print "Created admin mutex"
         
    return normal_mutex, admin_mutex